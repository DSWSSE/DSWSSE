...
/* 全局变量定义 */
int NUM_DOCS = 0;
int NUM_DOCS_BYTES = 0;
int MAX_DOCS = 0;
int MAX_DOCS_BYTES = 0;
int BLOOM_FILTER_SZ = 0;
int LOG_BLOOM_FILTER_SZ = 0;
int BLOOM_FILTER_BYTES = 0;
int MALICIOUS_DPF_LEN = 0;
...
void setBitOne(uint8_t *buf, int bitIndex) {
    if (bitIndex < 0 || bitIndex >= BLOOM_FILTER_SZ) {
        printf("ERROR: Index %d out of bounds (0-%d).\n", bitIndex, BLOOM_FILTER_SZ - 1);
        return;
    }
    int byteIndex = bitIndex / 8;
